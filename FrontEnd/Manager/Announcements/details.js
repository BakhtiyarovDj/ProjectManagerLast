    ...
    await axios.get(`${api_url}/${ann_id}`, {
        ...
        <div class="card" id="card-${ann_info.id}">
            <div class="card-header">
                ...
                <button class="btn btn-delete" onclick="deleteCard(${ann_info.id})">
                    <i class="bi bi-trash-fill"></i>
                </button>
            </div>
            <h2>Project Name: <span id="project_name" class="highlight">${ann_info.project_name}</span></h2>
            <div class="section">
                <p><strong>ID:</strong> <span id="id">${ann_info.id}</span></p>
            </div>
            <div class="section">
                <p><strong>Person:</strong> <span id="person" class="highlight">${ann_info.person}</span></p>
                <p><strong>Tashkilot Nomi:</strong> <span id="tashkilot_nomi" class="highlight">${ann_info.tashkilot_nomi}</span></p>
            </div>
            <div class="section">
                <p><strong>Address:</strong> <span id="address">${ann_info.address}</span></p>
                <p><strong>Phone:</strong> <span id="phone">${ann_info.phone}</span></p>
            </div>
            <div class="section">
                <p><strong>Name of Employer:</strong> <span id="name_of_employer">${ann_info.name_of_employer}</span></p>
                <p><strong>Person Name:</strong> <span id="person_name">${ann_info.person}</span></p>
            </div>
            <div class="section">
                <p><strong>Deadline:</strong> <span id="deadline">${ann_info.deadline}</span></p>
                <p><strong>Cost:</strong> <span id="cost">${ann_info.cost}</span></p>
            </div>
            <div class="section full-width">
                <p><strong>Description:</strong> <span id="description">${ann_info.description}</span></p>
                <p><strong>Assigned To:</strong> <span id="assigned_to">${team_id ? `<a href="../Teams/details.html?id=${team_id}">${ann_info.assigned_to_team}</a>` : 'Berilmagan'}</span></p>
            </div>
            <div class="section">
                <p><strong>Documents:</strong></p>
                <ul id="documents" style="width: 100%; padding-left: 0; display: flex; flex-wrap: wrap;">`;
            if (ann_info.document.file) {
                html += `
            <li>
                <div class="document-card">
                    <a href="${ann_info.document.file}">Document 1</a>
                </div>
            </li>`;
            }
            if (ann_info.document.file2) {
                html += `
            <li>
                <div class="document-card">
                    <a href="${ann_info.document.file2}">Document 2</a>
                </div>
            </li>`;
            }
            if (ann_info.document.file3) {
                html += `
            <li>
                <div class="document-card">
                    <a href="${ann_info.document.file3}">Document 3</a>
                </div>
            </li>`;
            }
            html += `
